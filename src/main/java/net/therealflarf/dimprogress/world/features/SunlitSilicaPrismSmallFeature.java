...
public class SunlitSilicaPrismSmallFeature extends Feature<NoneFeatureConfiguration> {
	...
	public SunlitSilicaPrismSmallFeature() {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		if (!generate_dimensions.contains(context.level().getLevel().dimension()))
			return false;
		if (template == null)
			template = context.level().getLevel().getStructureManager().getOrCreate(new ResourceLocation("dim_progress", "sunlit_silica_prism_small"));
		if (template == null)
			return false;
